...
import attackDefence from './attackDefence';
import distanceIndex from './distanceIndex';
...
export default class GameController {
  ...
  gameStart() {
    ...
    const characterPositions = getPositions(this.humanPositions.length);
  ...
  levelUp() {
    for (const item of this.humanPositions) {
      ...
      current.level += 1;
      current.attack = attackDefence(current.attack, current.health);
      current.defence = attackDefence(current.defence, current.health);
  ...
  computerMove(itemComputer) {
    ...
    if (Math.abs(nearTarget.steprow) === Math.abs(nearTarget.stepcolumn)) {
      if (Math.abs(nearTarget.steprow) > itemComputerDistance) {
        ...
        currentComputerCharacter.position = distanceIndex(distanceRow, distanceColumn);
      } else {
      ...
      if (Math.abs(nearTarget.steprow) > itemComputerDistance) {
        distanceRow = (itemComputerRow - (itemComputerDistance * Math.sign(nearTarget.steprow)));
        currentComputerCharacter.position = distanceIndex(distanceRow, (itemComputerColumn));
      } else {
        distanceRow = (itemComputerRow - (
          nearTarget.steprow - (1 * Math.sign(nearTarget.steprow))
        ));
      ...
      if (Math.abs(nearTarget.stepcolumn) > itemComputerDistance) {
        ...
        currentComputerCharacter.position = distanceIndex((itemComputerRow), distanceColumn);
      } else {
      ...
      currentComputerCharacter.position = distanceIndex((itemComputerRow), (itemComputerColumn));
    }
  }
